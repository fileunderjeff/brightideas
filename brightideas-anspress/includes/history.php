...
function ap_last_active_time($post_id = false, $html = true) {
	...
	// <span class="ap-post-history">'.sprintf( __('%s %s about %s ago', 'ap'), ap_user_display_name($history['user_id']), $title, '<time datetime="'. mysql2date('c', $history['date']) .'">'.ap_human_time( mysql2date('U', $history['date'])).'</time>' ).'</span>
...
//<a class="ap-avatar" href="'.ap_user_link($history['user_id']).'">'.get_avatar($history['user_id'], 22).'</a>
		$title = ap_history_title($history['type']);
		$html .= '';
		// <span class="ap-post-history">'.ap_icon('history', true).sprintf(__(' %s %s %s ago', 'ap'), ap_user_display_name($history['user_id']), $title, '<time datetime="'.mysql2date('c', $history['date']).'">'.ap_human_time($history['date'], false).'</time>').'</span>
	}
